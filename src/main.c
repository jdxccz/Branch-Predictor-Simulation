...
main(int argc, char *argv[])
{
  ...
  lcounter = s1.lcounter;
  rcounter = s1.rcounter;
  printf("L:%f  R:%f\n",(double)lcounter/((double)lcounter+(double)rcounter),(double)rcounter/((double)lcounter+(double)rcounter));
  // Cleanup
  fclose(stream);
  free(buf);
